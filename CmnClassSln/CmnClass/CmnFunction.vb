...
#Region "Get Year&Week From LotNo"
    '  Get Year from LotNo
    Public Shared Function GetYearPartStrFromLot(ByVal pstrLotNo As String) As String
        Dim strRet As String

        Try
            If Len(pstrLotNo) <> 10 Then
                Throw New Exception("Lot No is Error")
            End If
            strRet = pstrLotNo.Substring(1, 2)
    ...
    '  Get week from LotNo
    Public Shared Function GetWeekPartStrFromLot(ByVal pstrLotNo As String) As String
        Dim strRet As String
        ...
        Try
            ...
            strRet = pstrLotNo.Substring(3, 2)
...
#Region "Get Case From Product Code"
    '  Get Case Type of Product Code
    '  pStrProductCd = Assembly , Device , Sinter Product Code
    '  pStatus = A : Assembly
    '                     D : Device
    '                     S : Sinter
    Public Shared Function GetCasePartStrFromProductCD(ByVal pStrProductCD As String, _
        ...
        Try
            Select Case pStatus
                Case "A"                                                                                                     ' Assembly Product Code
                    strRet = pStrProductCD.Substring(5, 1)
                Case "D"                                                                                                    ' Device Product Code
                    strRet = pStrProductCD.Substring(1, 1)
                Case "S"                                                                                                    ' Sinter Product Code
                    strRet = pStrProductCD.Substring(8, 1)
                Case Else
...
#Region "Check Integer Data"
    '  Check Integer Data       
    Public Shared Function ChkIntergerData(ByVal pStrData As String) As Boolean

        Dim strRet As Boolean
        Dim i As Integer
        Dim xData As String
        Dim aryChar() As Char

        Try
            aryChar = New Char() {"1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "-"}    '  Set Number to Array
            strRet = True
            If Not pStrData Is Nothing And Len(pStrData) <> 0 Then
                For i = 0 To Len(pStrData) - 1                                                  ' Start Data 0
                    xData = pStrData.Substring(i, 1)
                    If xData.IndexOfAny(aryChar) < 0 Then
                        strRet = False
                        Exit For
                    End If
                Next
...
#Region "Check Double Data"
    '  Check Integer Data       
    Public Shared Function ChkDoubleData(ByVal pStrData As String) As Boolean
...
#Region " Function Translate Money change Number to String "
    ...
    Public Shared Function ConvertMoneyToString(ByVal pstrMoney As String) As String
        ...
        Dim intCount, LenFS1, LenFS2, Loop_Check, j, I, ChkFormat, ChkLp As Integer
        Dim strMoney, ThaiMoney, FractionStep1, FractionStep2, AddFormat As String
        ...
        Dim aryChar1(), aryChar2(), aryChar3(), aryChar4() As String
    ...
    Public Shared Function Thaimoney(ByVal Total As String) As String
        ...
        Dim Money_Count, MFS1, MFS2, Loop_Check, j, I, ChkFormat, ChkLp As Integer
...
#End Region
    ...
    ' Date format check(parameter is DataTime String)
    Public Shared Function IsDateTime(ByVal pstrCont As String) As Boolean
        Dim blnRet As Boolean
        Dim objDate As DateTime
        Try
            ' transfer String to DateTime
            objDate = Convert.ToDateTime(pstrCont)
            blnRet = True
        Catch ex As Exception
            blnRet = False
        End Try
    ...
    ' Date format check (parameter is DataTime As String and Format As String)
    Public Shared Function IsDateTime(ByVal pstrCont As String, ByVal pstrFormat As String) As Boolean
        Dim blnRet As Boolean
        Dim objDate As DateTime
        ...
        Try
            ...
            objDate = DateTime.ParseExact(pstrCont, pstrFormat, Nothing)
    ...
    ' Date Convert (parameter is DataTime As String and Format As String)
    Public Shared Function ToDateTime(ByVal pstrCont As String, ByVal pstrFormat As String) As DateTime
        ...
        Try
            ...
            ' check
            If CmnFunction.IsDateTime(pstrCont, pstrFormat) Then
                dteRet = DateTime.ParseExact(pstrCont, pstrFormat, Nothing)
    ...
    Public Shared Function GetString(ByVal pstrParam As String) As String
        Dim strRet As String

        Try
            ' initialize return value
            strRet = ""
            If Not pstrParam Is Nothing Then
                strRet = pstrParam
            End If
        Catch ex As Exception
            Throw ex
    ...
    ' convert String to Integer : if parameter is nothing or "" then return 0
    Public Shared Function GetInteger(ByVal pstrParam As String) As Integer
        Dim intRet As String

        Try
            ' initialize return value
            intRet = 0
            If Not pstrParam Is Nothing And pstrParam.Trim <> "" Then
                intRet = Convert.ToInt32(pstrParam)
            End If
        Catch ex As Exception
            Throw ex
        End Try
    ...
    ' parameter is Nothing or "" then return "0"
    Public Shared Function GetNumString(ByVal pstrCont As String) As String
        Dim strRet As String
        Try

            If Not pstrCont Is Nothing And pstrCont.Trim() <> "" Then
                strRet = pstrCont
            Else
                strRet = "0"
            End If
    ...
    ' get check digit in Code39
    Public Shared Function GetCheckDigit(ByVal pstrParam As String) As String
        Dim strRet As String
        Dim objMod As ArrayList
        Dim chaParam As Char()
        Dim intSum As Integer
        Dim intIndex As Integer

        Try
            ' create Code39 object
            objMod = New ArrayList
            objMod.Add("0")
            objMod.Add("1")
            objMod.Add("2")
            objMod.Add("3")
            ...
            ' target convert to char()
            chaParam = pstrParam.ToCharArray()

            ' get target sum number
            For i As Integer = 0 To chaParam.Length - 1
                For j As Integer = 0 To objMod.Count - 1
                    If chaParam(i).ToString().ToUpper = objMod(j).ToString().ToUpper Then
                        intSum = intSum + j
                        Exit For
                    End If
                Next
            ...
            ' calculate mod 43
            intIndex = intSum Mod 43

            ' set return value
            strRet = objMod(intIndex).ToString().ToUpper
    ...
    ' Trim + charactor ("+0601P0001T" -> "0601P0001T")
    Public Shared Function GetTrimPlus(ByVal pstrParam As String) As String
        Dim strRet As String
        Try
            strRet = pstrParam
            If pstrParam.StartsWith("+") Then
                strRet = pstrParam.Substring(1, pstrParam.Length - 1)
            End If
        Catch ex As Exception
            Throw ex
        End Try
    ...
    ' Folder Exist check(If not exist then Create)
    Public Shared Sub MakeFolder(ByVal pstrPath As String)
        Dim strRet As String
        Try
            If Not System.IO.Directory.Exists(pstrPath) Then
